 ...
 */
async function installWebKitGtk2Driver(version?: string, toPath?: string): Promise<string> {
    return new Promise((resolve: any, reject: any) => {
        ...
        if (!fs.existsSync(toPath)) {
            return reject(
                new Error(`WebKitGTK driver not found at ${toPath} try installing it manually with 'sudo apt-get install webkit2gtk-driver'`)
            )
        }
