 ...
 */
function capabilitiesForPlatform(platform?: string, webview?: string): selenium.Capabilities {
...
/**
 * Wrapper function for creating a {@link `selenium.Builder`} with the tauri binary path.
 * Note: Automatically strips the `.exe` extension if the platform is not `win32`.
 *
 ...
 *
 *  let builder = builderForBinary().build();
 *  ```
 *
 *  @param binaryPath The path to the tauri binary.
 */
function builderForBinary(binaryPath?: string): selenium.Builder {

    // get the capabilities for the current platform
    let capabilities = capabilitiesForPlatform();

    // create a new selenium builder
    let builder = new selenium.Builder()
        .withCapabilities(capabilities)

    // if the environment variable is set, override the server address
    if (process.env.SELENIUM_REMOTE_URL) {
        builder = builder.usingServer(process.env.SELENIUM_REMOTE_URL);
    }

    // if the environment variable is set, override the binary path
    if (process.env.TAURI_SELENIUM_BINARY) {
        binaryPath = process.env.TAURI_SELENIUM_BINARY;
    }
    ...
    // get the capabilities and set the binary path
    let caps = builder.getCapabilities();
    console.log(caps)
    // @ts-ignore
    let value = caps.get(capabilities.CAPABILITY_KEY);
    value.binary = binaryPath;
    // @ts-ignore
    caps.set(capabilities.CAPABILITY_KEY, value);

    logger.debug('Builder for binary', {
        "binary": binaryPath,
        "capabilities": caps,
    })
